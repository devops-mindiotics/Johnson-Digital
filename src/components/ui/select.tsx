    ...
    return React.Children.toArray(nodes)
      .map((node) => {
        ...
        if (node.type === SelectGroup) {
          const filteredChildren = filterNodes(node.props.children);
          if (React.Children.count(filteredChildren) > 0) {
        ...
        if (node.type === SelectItem) {
          const textValue = getTextValue(node.props.children);
  ...
  const getTextValue = (children: React.ReactNode): string => {
    ...
    React.Children.forEach(children, (child) => {
      if (typeof child === 'string') {
        ...
        text += getTextValue(child.props.children);
      }
    });
    return text;
  };
